...
- Plugin: **Update Time** — automatically updates the `created` and `updated` front-matter properties whenever a note changes, based on the file's `ctime` and `mtime`.
- Plugin id: `update-time`. Repo: https://github.com/dsebastien/obsidian-update-time.
- Source layout: `src/main.ts` (entry), `src/app/plugin.ts` (core class), `src/app/settingTab/index.ts` (settings UI), `src/app/utils/*` (helpers).
- Originally an Nx workspace (`apps/plugin/src/`, esbuild, npm, Jest). Migrated to the Bun-based template layout on **2026-04-15**: sources moved to `src/`, build via `scripts/build.ts`, tests via `bun test`, lint via `eslint.config.ts`.
...
- This plugin modifies vault files. Respect the user's exclusion list; never touch files in excluded folders. Debounce/throttle front-matter writes to avoid fighting active edits.
...
Issues and PRDs live in GitHub Issues (uses the `gh` CLI); external PRs are not a triage surface. See `docs/agents/issue-tracker.md`.
